...
class ZEDTracker(Node):
    def __init__(self):
        ...
        self.debug_pub = self.create_publisher(CompressedImage, '/zed/debug_image/compressed', 10)
        ...
        self.bottle_debug_pub  = self.create_publisher(CompressedImage, '/zed/bottle_debug/compressed',   10)

        # Publishers — pole
        self.pole_pub       = self.create_publisher(Point,           '/court_pole_position',          10)
        self.pole_debug_pub = self.create_publisher(CompressedImage, '/zed/pole_debug/compressed',    10)

        # TF broadcaster (for neon_bottle frame used by ball_chaser)
        self.tf_broadcaster = tf2_ros.TransformBroadcaster(self)

        self.get_logger().info("=" * 60)
        self.get_logger().info("ZED tracker ready  (ball + bottle + pole)")
    ...
    # =========================================================
    # FLOOR REMOVAL  (ZED depth is already in metres)
    # =========================================================
    def _remove_floor(self, cv_img):
        ...
        # ZED depth is already in metres — no /1000 conversion needed
        ...
        horizon_mask   = np.ones((h_img, w_img), dtype=bool)
    ...
    def _detect_ball(self, cv_img, hsv):
        ...
        nan_pct = int(np.sum(~np.isfinite(c_depth) | (c_depth <= 0))) / c_depth.size * 100.0

        contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        if not contours:
            self.get_logger().info("[ZED] No ball contours", throttle_duration_sec=1.0)
        ...
        self.get_logger().info(
            f"[ZED] Ball: z={z:.3f}m x={x_cam:.3f}m area={area:.0f}px",
            ...
            if not self.BOTTLE_ENABLE_VALIDATION:
                ...
                for c in contours:
                    ...
                    if self.depth_img is not None:
                        try:
                            raw = float(self.depth_img[
                                min(y_b + h_b // 2, self.depth_img.shape[0] - 1),
                                min(x_b + w_b // 2, self.depth_img.shape[1] - 1)])
                            if np.isfinite(raw):
                                rough_z = raw  # ZED depth is already in metres
                        except Exception:
                            pass

                    # Thresholds in metres (ZED depth is metres, not mm)
                    if rough_z > 3.0 or rough_z <= 0:
